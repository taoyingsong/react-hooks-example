...
2、除非你的自定义 hook 是作为【共享库中的一部分】才有价值。这样其他人更容易注意到你自定义的hook状态变化。
      </section>
      <section>
      <h3>请注意：</h3>
...
2、如果使用useDebugValue，最好设置第2个参数，向react开发调试工具讲清楚如何格式化展示第1个参数。因为格式化值的显示可能是一项开销很大的操作。该函数只有在 Hook 被检查时（就是在控制台实际查看时）才会被调用。它接受 debug 值作为参数，并且会返回一个格式化的显示值。
